...
/**
 * lcwins Child Theme functions and definitions
 *
 * @package lcwins Child Theme
 */

/**
 * Enqueue scripts and styles.  Grabs styles from parent theme for foundation inclusion.
 */
function lcwins_scripts()
{
	/* Add Google Fonts */
	...
	wp_enqueue_style( 'lcwins-style', get_stylesheet_directory_uri().'/style.css' );
}
add_action( 'wp_enqueue_scripts', 'lcwins_scripts' );
 ...
 */
function lcwins_add_editor_styles() {
	add_editor_style( 'style.css' );
}

add_action( 'admin_init', 'lcwins_add_editor_styles' );
...
register_nav_menus( array(
		'primary' => esc_html__( 'Primary Menu', 'lcwins' ),
		'secondary' => esc_html__( 'Secondary Menu', 'lcwins' ),
	) );
 ...
 */
function lcwins_widgets_init() {
				register_sidebar( array(
								'name'          => __( 'Sidebar', 'lcwins' ),
								'id'            => 'sidebar-1',
								'description'   => '',
						...
						register_sidebar( array(
								'name'          => __( 'Subpage Sidebar', 'lcwins' ),
								'id'            => 'subpagesidebar',
								'description'   => '',
								'before_widget' => '<aside id="%1$s" class="widget %2$s">',
								'after_widget'  => '</aside>',
...
add_action( 'widgets_init', 'lcwins_widgets_init' );
add_theme_support( 'post-thumbnails' );
